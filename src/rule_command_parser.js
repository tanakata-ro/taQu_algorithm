...
parseSingleCommand(content) {
    ...
    if (othersMatch) {
        ...
        return { type: 'others', mode: othersMatch[1].toLowerCase(), target: othersMatch[2], value: this.parseExpression(othersMatch[3]) };
    }
    ...
    if (uMatch) {
        ...
        return { type: 'others', mode: mode, target: uMatch[2], value: this.parseExpression(uMatch[3]) };
    }
    ...
    if (tMatch) {
        ...
        return { type: 'team', mode: mode, target: tMatch[2], value: this.parseExpression(tMatch[3]) };
    }
    ...
    if (assignMatch) {
        ...
        return { type: 'assign', var: v, op: eq || op, expr: this.parseExpression(exprStr) };
    }

    const statColorCommandMatch = content.match(/^([xyz])\.color\s*=\s*["'](.*?)["']$/);
    ...
    const setAddXYZMatch = content.match(/^(set|add)(X|Y|Z)\s*\(\s*(.+)\s*\)$/);
    if (setAddXYZMatch) {
    ...
    if (broadcastMatch) {
        ...
        return { type: 'broadcast', target: broadcastMatch[1], value: this.parseExpression(broadcastMatch[2]) };
    }

    const resetVarMatch = content.match(/^resetVar\s*\(\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*\)$/);
    if (resetVarMatch) {
        this.assertWritableName(resetVarMatch[1], 'resetVar');
        return { type: 'resetVar', var: resetVarMatch[1] };
    }
